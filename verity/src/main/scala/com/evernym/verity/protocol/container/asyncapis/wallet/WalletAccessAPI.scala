...
import com.evernym.verity.protocol.engine.asyncService.wallet.{InvalidSignType, WalletAccess}
...
class WalletAccessAPI(protected val appConfig: AppConfig,
                     ...
                     (implicit val wap: WalletAPIParam, asyncAPIContext: AsyncAPIContext)
  ...
  override def verify(signer: ParticipantId,
    ...
    if (signType != SIGN_ED25519_SHA512_SINGLE) {
      ...
      withAsyncOpRunner(
        {
          getVerKeyFromParticipantId(signer).flatMap { gvkr =>
            if (verKeyUsed.getOrElse(gvkr.verKey).contains(gvkr.verKey)) {
              verifyBase(msg, sig, gvkr.verKey, signType)
            } else {
  ...
  // we check the VerKey used belongs to the party who signed the message.
    withAsyncOpRunner({verifyBase(msg, sig, verKeyUsed, signType)}, handler)
  }

  private def verifyBase(msg: Array[Byte],
                         sig: Array[Byte],
                         verKeyUsed: VerKey,
                         signType: SignType): Future[VerifySigResult] = {
    // libindy currently supports only one VerKey per DID
    // we check the VerKey used belongs to the party who signed the message.
    val toVerify = VerifySigByVerKey(verKeyUsed, msg, sig)
    CryptoOpExecutor.verifySig(toVerify).map(_.verified)
      .map(r => VerifySigResult(r))
  }

  private def getVerKeyFromParticipantId(participantId: ParticipantId): Future[GetVerKeyResp] = {
    val did = getDIDFromParticipantId(participantId)
    walletApi.executeAsync[GetVerKeyResp](GetVerKey(did))
  }
