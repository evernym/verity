...
object Libraries {
  ...
  def initialize(appConfig: AppConfig): Unit = {
    ...
    val libIndyDirPath: String = {
      val lifp = appConfig.getStringReq(ConfigConstants.LIB_VDRTOOLS_LIBRARY_DIR_LOCATION)
    ...
    && appConfig.getStringReq(ConfigConstants.LIB_VDRTOOLS_FLAVOR) == LIBINDY_LEGACY_FLAVOR) {
      try {
