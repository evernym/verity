...
//import com.evernym.verity.actor.wallet.{CreateMasterSecret, CreateNewKey, CredCreated, CredDefCreated, CredForProofReqCreated, CredOfferCreated, CredReqCreated, CredStored, MasterSecretCreated, NewKeyCreated, SignMsg, SignedMsg}
...
//
//  var trusteeWallet = new TestWallet(ec, createWallet = true, actorSystem)
...
//
//  var issuerWalletExt = new TestWallet(ec, createWallet = true, actorSystem)
...
//
//  var verifierWalletExt = new TestWallet(ec, createWallet = true, actorSystem)
...
//
//  val holderWalletExt = new TestWallet(ec, createWallet = true, actorSystem)
...
//        val pingResult = runAsSync(vdrAdapter.ping(List("sov")))
//        pingResult.status shouldBe Map("sov" -> LedgerStatus(reachable = true))
//      }
//    }
//
...
//          signAndSubmitTxn(preparedTxn)
//        }
//        ex.getMessage shouldBe "No consensus was reached during the ledger operation."
//      }
//    }
//
...
//      "should be successful" in {
////        val preparedTxn = runAsSync {
////          vdrAdapter.prepareDidTxn(s"""{"dest":"$issuerFqDid", "verkey": "${issuerKey.verKey}", "role": "ENDORSER" }""", trusteeFqDid, None)
////        }
////
////        val signedMsg = trusteeWallet.executeSync[SignedMsg](SignMsg(KeyParam.fromVerKey(trusteeKey.verKey), preparedTxn.bytesToSign))
////
////        val submittedTxn = runAsSync {
...
//        schema.fqId.startsWith("schema:sov:did:sov:") shouldBe true
//        schema.fqId.endsWith(":2:employment:1.0") shouldBe true
//
//        val preparedTxn = runAsSync {
//          val configJson = "{}"
...
//        val credStored = runAsSync(Anoncreds.proverStoreCredential(holderWallet,
//          credDefCreated.fqId, credReqCreated.credReqMetadataJson, attachedCred.toString(), credDefCreated.json, null)
//          .map (c => CredStored(c)))
//      }
//    }
//
...
//        val credentialsNeeded = creds.map(_.cred).map(DefaultMsgCodec.fromJson[AvailableCredentials](_))
//        val (credentialsUsedJson, ids) = credentialsToUse(credentialsNeeded, Map.empty)
//        val credUsedJSONObject = new JSONObject(credentialsUsedJson.get)
//        val reqAttributesJSONObject = credUsedJSONObject.getJSONObject("requested_attributes")
//        val attrsJSONObject = reqAttributesJSONObject.getJSONObject("name:company")
//        attrsJSONObject.getString("cred_id").startsWith("creddef:sov:did:sov:") shouldBe true
//        attrsJSONObject.getString("cred_id").contains(":3:CL:") shouldBe true
//        attrsJSONObject.getString("cred_id").endsWith(":latest") shouldBe true
...
//  }
//
//  private def signAndSubmitTxn( preparedTxn: PreparedTxn): SubmittedTxn = {
//    val signedMsg = issuerWalletExt.executeSync[SignedMsg](SignMsg(KeyParam.fromVerKey(issuerKey.verKey), preparedTxn.bytesToSign))
//
//    runAsSync {
//      vdrAdapter.submitTxn(
//        preparedTxn,
