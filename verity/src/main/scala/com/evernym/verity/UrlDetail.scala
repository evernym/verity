...
object UrlDetail {
  ...
  def apply(host: String, port: Int, path: Option[String]): UrlDetail = {
    UrlDetail(protocolToBeUsed(port), host, port, path, None)
  }

  def apply(urlStr : String): UrlDetail = {
    try {
      ...
      UrlDetail(url.getProtocol, url.getHost, port, path, query)
    } catch {
...
case class UrlDetail(protocol: String, host: String, port: Int, private val pathOpt: Option[String], query: Option[String]=None) {
  import UrlDetail._
