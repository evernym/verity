...
object MessageBehaviour {
  ...
  def apply(entityContext: EntityContext[Cmd], storageAPI: StorageAPI): Behavior[Cmd] = {
    EventSourcedBehaviorBuilder
      .default(
        PersistenceId(TypeKey.name, entityContext.entityId),
        States.Uninitialized,
        commandHandler(storageAPI),
        eventHandler)
      .withEventCodeMapper(EventObjectMapper)
      .build()
  }

  private def commandHandler(storageAPI: StorageAPI)(util: BehaviourUtil): (State, Cmd) => ReplyEffect[Any, State] = {
