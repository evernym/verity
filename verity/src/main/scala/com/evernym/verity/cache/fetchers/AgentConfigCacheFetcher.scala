...
class AgentConfigCacheFetcher(val agentMsgRouter: AgentMsgRouter, appConfig: AppConfig) extends AsyncCacheValueFetcher {
  ...
  lazy val expiryTimeInSeconds: Option[Int] = Option(appConfig.getConfigIntOption(AGENT_CONFIG_CACHE_EXPIRATION_TIME_IN_SECONDS).getOrElse(300))
  lazy val maxSize: Option[Int] = appConfig.getConfigIntOption(AGENT_CONFIG_CACHE_MAX_SIZE)
