...
object AnoncredsWalletOpExecutor extends OpExecutorBase {
  ...
  def handleCreateMasterSecret(cms: CreateMasterSecret)(implicit we: WalletExt, ec: ExecutionContext): Future[MasterSecretCreated] = {
    Anoncreds
      .proverCreateMasterSecret(we.wallet, cms.masterSecretId)
  ...
  def handleCreateCredDef(ccd: CreateCredDef)(implicit we: WalletExt, ec: ExecutionContext): Future[CredDefCreated] = {
    ...
    Anoncreds
      .issuerCreateAndStoreCredentialDef(
        ...
        ccd.schemaJson,
        ccd.tag,
        ccd.sigType.getOrElse(SIG_TYPE_CL),
        configJson)
  ...
  def handleCreateCredOffer(cco: CreateCredOffer)(implicit we: WalletExt, ec: ExecutionContext): Future[CredOfferCreated] = {
    Anoncreds
      .issuerCreateCredentialOffer(we.wallet, cco.credDefId).map(co => CredOfferCreated(co))
      .recover {
        case _: WalletItemNotFoundException =>
          throw new BadRequestErrorException(
            DATA_NOT_FOUND.statusCode,
            Option(s"cred def '${cco.credDefId}' not found in wallet"))
      }
  }

  def handleCreateCredReq(ccr: CreateCredReq)(implicit we: WalletExt, ec: ExecutionContext): Future[CredReqCreated] = {
    Anoncreds
      .proverCreateCredentialReq(we.wallet, ccr.proverDID, ccr.credOfferJson, ccr.credDefJson, ccr.masterSecretId)
  ...
  def handleCreateCred(cc: CreateCred)(implicit we: WalletExt, ec: ExecutionContext): Future[CredCreated] = {
    Anoncreds
      .issuerCreateCredential(we.wallet, cc.credOfferJson, cc.credReqJson, cc.credValuesJson, cc.revRegistryId, cc.blobStorageReaderHandle)
  ...
  def handleStoreCred(sc: StoreCred)(implicit we: WalletExt, ec: ExecutionContext): Future[CredStored] = {
    Anoncreds
      .proverStoreCredential(we.wallet, sc.credId, sc.credReqMetadataJson, sc.credJson, sc.credDefJson, sc.revRegDefJson)
  ...
  def handleCredForProofReq(cfpr: CredForProofReq)(implicit we: WalletExt, ec: ExecutionContext): Future[CredForProofReqCreated] = {
    Anoncreds
      .proverGetCredentialsForProofReq(we.wallet, cfpr.proofRequest)
  ...
  def handleCreateProof(cp: CreateProof)(implicit we: WalletExt, ec: ExecutionContext): Future[ProofCreated] = {
    Anoncreds
      .proverCreateProof(we.wallet, cp.proofRequest, cp.requestedCredentials, cp.masterSecret,
  ...
  def verifyProof(proofRequest: String, proof: String, schemas: String, credentialDefs: String,
    ...
    Anoncreds
      .verifierVerifyProof(proofRequest, proof, schemas, credentialDefs, revocRegDefs, revocRegs)
