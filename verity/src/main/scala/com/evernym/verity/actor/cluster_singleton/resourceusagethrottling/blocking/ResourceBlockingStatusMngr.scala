...
class ResourceBlockingStatusMngr(val aac: AgentActorContext)
  ...
  override val receiveCmd: Receive = LoggingReceive.withLabel("receiveCmd") {
    ...
    case bur: BlockResourceForCaller =>
    ...
    case uur: UnblockResourceForCaller =>
 ...
 */
case class EntityBlockingStatus(status: BlockingDetail, resourcesStatus: Map[ResourceName, BlockingDetail]) extends ActorMessageClass
