...
import com.evernym.verity.vdr.{CacheOption, CredDef, CredDefId, DidDoc, FqCredDefId, FqDID, FqSchemaId, LedgerStatus, Namespace, PingResult, PreparedTxn, Schema, SchemaId, SubmittedTxn, VDRAdapter}
...
class MockVDRAdapter(vdrToolsFactory: VDRToolsFactory)(implicit executionContext: ExecutionContext)
  ...
  override def prepareSchemaTxn(schemaJson: String,
                                ...
                                submitterDID: FqDID,
                                endorser: Option[String]): Future[PreparedTxn] =
    vdrTools.prepareSchema(buildVDRSchemaParams(schemaJson, schemaId), submitterDID, endorser)
      .map(r => VDRAdapterUtil.buildPreparedTxn(r))
  ...
  override def prepareCredDefTxn(credDefJson: String,
                                 ...
                                 submitterDID: FqDID,
                                 endorser: Option[String]): Future[PreparedTxn] =
    vdrTools.prepareCredDef(buildVDRCredDefParams(credDefJson, credDefId), submitterDID, endorser)
      .map(r => VDRAdapterUtil.buildPreparedTxn(r))
