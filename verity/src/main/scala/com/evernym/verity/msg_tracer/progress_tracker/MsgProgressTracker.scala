...
trait MsgProgressTracker extends HasActorResponseTimeout { this: MsgTraceProvider =>
  ...
  def trackingTheirRelationshipId: Option[RelationshipId] = None
  ...
  def relTrackingId: Option[String] = {
    if (trackingMyRelationshipId == trackingDomainId) None    //this is self relationship
      ...
      val tid = Set(trackingMyRelationshipId, trackingTheirRelationshipId).toSeq.sorted.mkString(":")
  ...
  object MsgProgressTracker {
    ...
    def recordEventForTrackingIds(clientIpAddress: String, recordEvent: RecordEvent): Future[Any] = {
    ...
    private def checkAndRecord(trackingId: String, recordEvent: RecordEvent): Unit =
      if (isTrackingStarted(trackingId)) {
        sendToProgressTrackerRegion(trackingId, recordEvent)
      }

    private def isTrackingStarted(trackingId: String): Boolean =
      MsgProgressTrackerCache.isTracked(trackingId)

    def getRecordedRequests(trackingId: String, reqId: Option[String]=None,
                            domainTrackingId: Option[String], relTrackingId: Option[String],
    ...
    def recordProtoMsgStatus(protoDef: ProtoDef,
                             pinstId: String,
                             context: String,
                             reqId: String, inMsg: Option[Any]=None, outMsg: Option[Any]=None): Unit = {
      if (isTrackingStarted(pinstId)) {
        val inMsgName = inMsg.map(getMsgName)
        val outMsgName = outMsg.map(getMsgName)
        sendToProgressTrackerRegion(pinstId, RecordEvent(reqId,
          EventParam(EventProtocolMsgProgress(
            context,
            inMsgParam = MsgParam(msgName = inMsgName),
            outMsgParam = MsgParam(msgName = outMsgName),
    ...
    def recordMsgUnpackedByAgent(additionalContext: String, inMsgParam: MsgParam=MsgParam())
                                (implicit rmc: ReqMsgContext): Unit = {
      rmc.clientIpAddress.foreach { cip =>
    ...
    def recordOutMsgPackagingStarted(inMsgParam: MsgParam=MsgParam(),
                                     outMsgParam: MsgParam=MsgParam()): Unit = {
    ...
    def recordOutMsgPackagingFinished(inMsgParam: MsgParam=MsgParam(),
                                      outMsgParam: MsgParam=MsgParam()): Unit = {
      getAsyncReqContext(inMsgParam.msgId, outMsgParam.msgId).foreach { arc =>
        arc.clientIpAddress.foreach { cip =>
    ...
    def recordLegacyRespMsgPackagingFinished(outMsgParam: MsgParam = MsgParam())
                                            (implicit rmc: ReqMsgContext): Unit = {
      rmc.clientIpAddress.foreach { cip =>
        val event = EventOutMsgPackagingFinished(outMsgParam = outMsgParam)
        recordEvent(cip, rmc.id, event)
    ...
    def recordLegacyMsgSendingFailed(nextHop: String, errorMsg: String, replyToMsgId: Option[MsgId]=None)
                                    (implicit rmc: ReqMsgContext): Unit = {
      rmc.clientIpAddress.foreach { cip =>
        val event = EventOutMsgSendingFailed(hop=nextHop, errorMsg=errorMsg)
        recordEvent(cip, rmc.id, event)
    ...
    private def getAsyncReqContext(reqMsgId: Option[ReqMsgId]=None, respMsgId: Option[RespMsgId]=None): Option[AsyncReqContext] = {
  ...
  private def sendToProgressTrackerRegion(trackingId: String, msg: Any): Future[Any] = {
