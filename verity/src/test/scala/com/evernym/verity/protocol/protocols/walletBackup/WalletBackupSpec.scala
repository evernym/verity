...
import com.evernym.verity.protocol.engine.{DebugProtocols, SimpleProtocolSystem, VerKey}
...
class WalletBackupSpec()
  ...
  private implicit def EnhancedScenario(s: Scenario) = new {
    val exporter: TestEnvir = s(EXPORTER)
    val persister: TestEnvir = s(PERSISTER)
  }
    ...
    "init tests" - {
      ...
      "when persister registers recovery key" - {
        "should result in roles being set and WalletInitialized" in autoRegisteredVk { f =>
          ...
          f(PERSISTER).state shouldBe a[State.ReadyToPersistBackup]
    ...
    "uploading tests" - {
      "when backup is uploaded" - {
        ...
        "should be able to overwrite with new wallet (byte array)" in autoRegisteredVk { f =>
          ...
          f(PERSISTER).state.asInstanceOf[State.ReadyToPersistBackup]
        }
