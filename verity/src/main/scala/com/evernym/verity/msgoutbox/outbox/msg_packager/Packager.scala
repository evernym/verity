...
import com.evernym.verity.msgoutbox.outbox.msg_packager.Packager.Replies.{DIDCommV1PackedMsg, NoPackedMsg}
...
object Packager {
  ...
  object Replies {
    case class NoPackedMsg(msg: String) extends Reply
    ...
    msg.payload match {
      ...
      case Some(payload) =>
        ...
        if (recipPackaging.pkgType == MPF_PLAIN.toString) {
          replyTo ! NoPackedMsg(new String(payload))
          Behaviors.stopped
          ...
          val didCommV1Packager = actorContext.spawnAnonymous(msgPackagingParam.packagers.didCommV1Packager)
