...
// when senderName is explicitly provided during send message
// in this case the provided name should be used as a 'senderName' in the push notification
// irrespective of whether it is set in agent configuration or not

class PushNotifWithOverriddenSenderNameSpec
  ...
  "IssuerSDK" - {
    "when tried to send 'ask-question' (questionanswer 1.0) message" - {
      "should be successful" in {
        ...
        issuerSDKEAS.sendCreateMsgReq_0_5(connId, packedMsg, Option("pairwise-issuer-name"))
      }
    }
  }
    ...
    "when tried to check push notification" - {
      "should receive it" in {
        checkPushNotif { pushNotifJson =>
          ...
          pushNotifMsgText.startsWith("pairwise-issuer-name sent you a general") shouldBe true
        }
      }
    }
  }
