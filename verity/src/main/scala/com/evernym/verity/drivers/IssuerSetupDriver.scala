...
import com.evernym.verity.protocol.protocols.issuersetup.v_0_6.{GetIssuerIdentifier, ProblemReport, PublicIdentifier, PublicIdentifierCreated}
...
class IssuerSetupDriver(cp: ActorDriverGenParam, ec: ExecutionContext) extends ActorDriver(cp, ec) {
  override def signal[A]: SignalHandler[A] = {
    case sig@SignalEnvelope(_: GetIssuerIdentifier, _, _, _, _) =>
