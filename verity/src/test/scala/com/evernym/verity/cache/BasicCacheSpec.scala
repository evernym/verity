...
import com.evernym.verity.cache.base.{Cache, QueryResult, FetcherParam, GetCachedObjectParam, ReqParam}
...
class BasicCacheSpec
    ...
    "when asked for non existing key" - {
      "should get empty map" in {
        getFromCache(cache, "a", required = false).map { cqr =>
          cqr shouldBe QueryResult(Map.empty)
          cache.allCacheSize shouldBe 0
    ...
    "when asked key value mapper object with key 'a'" - {
      "should get it from source and save it in cache" in {
        ...
        getFromCache(cache, "a", required = true).map { cqr =>
          cqr shouldBe QueryResult(Map("a" -> "apple"))
  ...
  "when asked for multiple values from cache" - {
    "should get it from from cache" in {
      ...
      getFromCache(cache, Set(ReqParam("a", required = true), ReqParam("b", required = true))).map { cqr =>
        cqr shouldBe QueryResult(Map("a" -> "apple", "b" -> "berry"))
  ...
  "when source is updated with new value" - {
    "should respond with updated value after expiry time" in {
      ...
      Thread.sleep(1100)
      getFromCache(cache, Set(ReqParam("a", required = true), ReqParam("b", required = true))).map { cqr =>
        cqr shouldBe QueryResult(Map("a" -> "apple", "b" -> "blackberry"))
        cache.allCacheSize shouldBe 2
  ...
  def getFromCache(cache: Cache, key: String, required: Boolean): Future[QueryResult] = {
    getFromCache(cache, Set(ReqParam(key, required)))
  }

  def getFromCache(cache: Cache, keyDetails: Set[ReqParam]): Future[QueryResult] = {
