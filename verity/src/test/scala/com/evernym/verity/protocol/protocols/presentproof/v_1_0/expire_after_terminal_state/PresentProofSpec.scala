...
class PresentProofSpec
  ...
  override val defaultInitParams = Map(
      ...
      DATA_RETENTION_POLICY -> s"""{"expire-after-days":"100 day", "expire-after-terminal-state": true}"""
  )
    ...
    "verifier start protocol" - {
      "should handle happy path" in { f =>
        ...
        verifier.expectAs(signal[Sig.StatusReport]) { s =>
          ...
          s.results shouldBe None   //because expire-after-terminal-state config
  ...
  override def assertProofReceived(env: TestEnvir, segmentKey: SegmentKey): Unit = {
    env.container_!.withSegment[PresentationGiven](segmentKey) {
      case Success(Some(r)) => throw new Exception("unexpected result (segment state should have been removed at this point)")
