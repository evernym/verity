...
import com.evernym.verity.http.route_handlers.restricted.{AbstractApiHealthChecker, ApiStatus, ReadinessStatus}
import org.mockito.MockitoSugar.when

import scala.concurrent.Future


class MockApiHealthChecker extends AbstractApiHealthChecker{
...
trait ApiHealthCheckSpec {this: EdgeEndpointBaseSpec =>
  ...
  def testBaseApiHeathCheck(): Unit = {
    "when sent req to /verity/node/readiness and event journal and wallet storage services ready" - {
      "should be return 200 OK" in {
        when(apiHealthChecker.checkAkkaEventStorageReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        when(apiHealthChecker.checkWalletStorageReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        when(apiHealthChecker.checkStorageAPIReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
    ...
    "when sent req to /verity/node/readiness and only event journal isn't responding" - {
      "should be return 503 Unavailable" in {
        when(apiHealthChecker.checkAkkaEventStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "Something bad")))
        when(apiHealthChecker.checkWalletStorageReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        when(apiHealthChecker.checkStorageAPIReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        buildGetReq("/verity/node/readiness") ~> epRoutes ~> check {
    ...
    "when sent req to /verity/node/readiness and only wallet storage isn't responding" - {
      "should be return 503 Unavailable" in {
        when(apiHealthChecker.checkAkkaEventStorageReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        when(apiHealthChecker.checkWalletStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
        when(apiHealthChecker.checkStorageAPIReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
        buildGetReq("/verity/node/readiness") ~> epRoutes ~> check {
          status shouldBe ServiceUnavailable
  ...
  "when sent req to /verity/node/readiness and event journal and wallet storage isn't responding" - {
    "should be return 503 Unavailable" in {
      when(apiHealthChecker.checkAkkaEventStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
      when(apiHealthChecker.checkWalletStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
      when(apiHealthChecker.checkStorageAPIReadiness).thenReturn(Future.successful(ApiStatus(true, "OK")))
      buildGetReq("/verity/node/readiness") ~> epRoutes ~> check {
        status shouldBe ServiceUnavailable
  ...
  "when sent req to /verity/node/readiness and event journal, wallet storage and storage api isn't responding" - {
    "should be return 503 Unavailable" in {
      when(apiHealthChecker.checkAkkaEventStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
      when(apiHealthChecker.checkWalletStorageReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
      when(apiHealthChecker.checkStorageAPIReadiness).thenReturn(Future.successful(ApiStatus(false, "BAD")))
      buildGetReq("/verity/node/readiness") ~> epRoutes ~> check {
        status shouldBe ServiceUnavailable
  ...
  "when sent req to /verity/node/liveness and all it's ok" - {
    "should be return 200 OK" in {
      when(apiHealthChecker.checkLiveness).thenReturn(Future.successful((): Unit))
  ...
  "when sent req to /verity/node/liveness and verity is not 'healthy'" - {
    "should be return 503 Unavailable" in {
      when(apiHealthChecker.checkLiveness).thenReturn(Future.failed(new Exception("BAD")))
      buildGetReq("/verity/node/liveness") ~> epRoutes ~> check {
        status shouldBe ServiceUnavailable
