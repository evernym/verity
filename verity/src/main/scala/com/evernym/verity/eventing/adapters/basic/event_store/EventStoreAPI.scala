package com.evernym.verity.eventing.adapters.basic.event_store
...
import com.evernym.verity.eventing.adapters.basic.HttpServerParam
...
class BasicEventStoreAPI(config: Config)(implicit system: ActorSystem, executionContext: ExecutionContext)
  ...
  private val listenerParam: HttpServerParam = {
    HttpServerParam(
      config.getString("verity.eventing.basic-store.http-listener.host"),
      config.getInt("verity.eventing.basic-store.http-listener.port")
    )
  }
